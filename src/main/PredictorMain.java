...
import infrastructure.ConnectToDatabase;
import infrastructure.InsertIntoDatabase;
import infrastructure.PrepareData;
import infrastructure.ReadData;
import infrastructure.TrainOrUseNeuronalNet;

/**
 * The main class for Predictor, basically the the database will be connected,
 * the data to learn the network will be read by an sql file. The training will
 * be set up and the results are presented. Finally the results can be stored in
 * the database.
 * 
...
public class PredictorMain {
	...
	private static PreProAiContext preProAiContext = new PreProAiContext();
	private static NeuronalNetworkCogNetContext neuronalNetworkCogNetContext = new NeuronalNetworkCogNetContext();
	private static ReadData readData = new ReadData();
	private static PrepareData prepareData = new PrepareData();
	private static TrainOrUseNeuronalNet trainOrUseNeuronalNet = new TrainOrUseNeuronalNet();
	private static InsertIntoDatabase insertIntoDatabase = new InsertIntoDatabase();
	private static ConnectToDatabase connectToDatabase = new ConnectToDatabase();

	private static Logger log = Logger.getLogger(PredictorMain.class.getName());

	public static void main(String[] args) {

		log.log(Level.INFO, "Predictor started", "");

		/**
		 * Step 1: Read all data for running a prediction, parameter-file,
		 * database-file and the data from the database as described in the
		 * SQL-file
		 */
		readData.readData(preProAiContext, neuronalNetworkCogNetContext, args);

		/**
		 * Step 2: Analyze the input-data and prepare the data for a neuronal
		 * net. Finally translate it into the proper format for ENCOG.
		 */
		prepareData.preapareForNeuronalNet(preProAiContext, neuronalNetworkCogNetContext);

		if (!preProAiContext.isOnlyPrepare()) {
			/**
			 * Step 3: Either the training mode for learning patterns
			 */
			if (preProAiContext.getTrainOrApplyMode().equals(PreProAiContext.getTrainmode()))
				trainOrUseNeuronalNet.trainAndTestNeuronalNet(preProAiContext, neuronalNetworkCogNetContext);

			/**
			 * Step 3: Or the apply-mode for prediction, using an already
			 * trained network
			 */
			if (preProAiContext.getTrainOrApplyMode().equals(PreProAiContext.getApplymode()))
				trainOrUseNeuronalNet.predictWithNeuronalNet(preProAiContext, neuronalNetworkCogNetContext);

			/**
			 * Step 4: Writes the predicted data back into the database
			 */
			if (preProAiContext.isWriteIntoDatabase())
				insertIntoDatabase.inserPredictionIntoDatabase(preProAiContext, neuronalNetworkCogNetContext);

			/**
			 * Step 5: shutdown for ENCOG
			 */
			Encog.getInstance().shutdown();

			/**
			 * Step 6: Close connection to database
			 */
			connectToDatabase.disconnectFromDatabase(preProAiContext);
		}

		log.log(Level.INFO, "Predictor finished", "");
	}

}

/**
 * \mainpage Welcome to Predictor
 * 
